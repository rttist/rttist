...
export function getCallSignatures(type: ts.Type, context: Context): SignatureProperties[] {
	return type.getCallSignatures().map((signature) => ({
		...
		returnType: TransformerTypeReference.Invalid,
		...
		typeParameters: signature.typeParameters?.map(
			(typeParameter) => TransformerTypeReference.Invalid
			// context.metadata.referenceType(typeParameter, false, undefined, undefined, context)
		),
	}));
}
