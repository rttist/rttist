...
export abstract class ObjectLikeTypeBase extends Type {
	...
	protected constructor(initializer: ObjectLikeBaseTypeMetadata) {
		...
		// @ts-ignore
		this._isIterable =
			this._properties?.some((prop) => prop.name.isSymbol() && prop.name.name === Symbol.iterator) ||
			this._methods?.some((method) => method.name.isSymbol() && method.name.name === Symbol.iterator);
