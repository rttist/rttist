...
import { Metadata, getType } from "rttist/typelib"; // this import is "virtual"; generated by TypeGen
...
Module `rttist/typelib` is generated by TypeGen (file `.metadata/metadata.typelib.ts`). It exports `Metadata` object which is instance of the `MetadataLibrary`. It contains metadata for all types and modules in the project.
Vite plugin handles this import automatically, so you can use it without any additional configuration. With vanilla TypeScript, you need to run `typegen generate` and create path alias for `rttist/typelib` in your `tsconfig.json` file.
